...
func (client *ApiReq) DdkOrderListRangeGet(startTime, endTime, lastOrderId string, pageSize int64) (*OrderList, *ApiErrorInfo) {
	params := ApiParams{}
	params["start_time"] = startTime
	params["end_time"] = endTime
	if lastOrderId != "" {
		params["last_order_id"] = lastOrderId //上一次的迭代器id(第一次不填)
	}
	if pageSize > 0 {
		params["page_size"] = fmt.Sprint(pageSize) //每次请求多少条，建议300
	} else {
		params["page_size"] = "300"
	}
	resp, err := client.Execute("pdd.ddk.order.list.range.get", params)
	if err != nil {
		return nil, err
	}
	...
	if orderResp.Exists() && orderResp.Get("order_list").IsArray() {
		...
		orderList.OrderList = orderDetails
		orderList.LastOrderId = orderResp.Get("last_order_id").String()
		return &orderList, nil
	} else {
		errInfo := ApiErrorInfo{}
		errInfo.ErrorCode = 77777
